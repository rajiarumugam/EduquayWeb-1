...
@Component({
  selector: 'app-users-molecular',
  templateUrl: './users-molecular.component.html',
  styleUrls: ['./users-molecular.component.css']
})
export class UsersAdminMolecularComponent implements OnInit {
...
selectedChc: string;
  ...
  ngOnInit() {
    ...
    this.dataservice.sendData(JSON.stringify({"module": "Users", "submodule": "Molecular"}));
  ...
  ddlUserRole() {
    ...
    let district = this.UsersService.getUserroleListType(9).subscribe(response => {
  ...
  refreshData()
      {
        ...
        this.UsersService.getUsersList(9).subscribe(response => {
