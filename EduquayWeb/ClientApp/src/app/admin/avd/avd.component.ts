...
import { AddAvdpointResponse,AddAvdPtDataresponse,AvdList } from 'src/app/shared/admin/add-avd/add-avd-response';
...
import { AddAvdpointService } from 'src/app/shared/admin/add-avd/add-avd.service';
...
export class AVDComponent implements AfterViewInit, OnDestroy, OnInit {
   ...
   tempchcname:string;
    ...
    avdptListRequest;
    addilrptResponse: AddAvdPtDataresponse;
    chcListResponse;
    chclists: ChcList[];
    phcListResponse;
    phclists: PhcList[];
  ...
  ilrptlists: AvdList[];
  AVDPtListResponse: AddAvdpointResponse;
  
    constructor(
    
      private  AvdPtService: AddAvdpointService,
    ...
    retrieveRiPtList(){
      ...
      let samplesList = this.AvdPtService.getRiList()
      .subscribe(response => {
        this.AVDPtListResponse = response;
        console.log(response)
        this.loaderService.display(false);
        if(this.AVDPtListResponse !== null){
          console.log("does not get data");
          if(this.AVDPtListResponse.avdDetails.length <= 0){
            this.ripointlistErrorMessage = response.message;
            
          }
          else{
            this.ilrptlists = this.AVDPtListResponse.avdDetails;
    ...
    districtChange()
    {
          ...
          this.selectedChc = '';
      let district = this.AvdPtService.getCHCByDis(this.selectedDistrict).subscribe(response => {
    ...
    districteditChange()
    {
          ...
          this.selectedChc = '';
      let district = this.AvdPtService.getCHCByDis(this.selectedEditDistrict).subscribe(response => {
        this.chcListResponse = response;
        if (this.chcListResponse !== null && this.chcListResponse.status === "true") {
          this.chclists = this.chcListResponse.data;
          this.selectedChc = "";
    ...
    ddlDistrict() {
      let district = this.AvdPtService.getDistrictList().subscribe(response => {
    ...
    ddlChc() {
      ...
      this.selectedChc = '';
      let district = this.AvdPtService.getChcList().subscribe(response => {
        this.chcListResponse = response;
        if (this.chcListResponse !== null && this.chcListResponse.status === "true") {
    ...
    ddlPhc(code) {
      this.selectedPhc = '';
      let district = this.AvdPtService.getPhcList(code).subscribe(response => {
    ...
    ddlSc(code) {
      this.selectedPhc = '';
      let district = this.AvdPtService.getScList(code).subscribe(response => {
    ...
    openEditRiPt(editRiPtDetail, sample:AvdList) {
  
    
      this.Tempeditid=sample.id
     
      this.avdName= sample.avdName;
      this.avdContact = String(sample.contactNo);
      this.avdRiid=sample.riId;
    ...
    onSubmit(addRiPtForm: NgForm){
      ...
      this.riId = addRiPtForm.value.riId;
      ...
      this.ilrptListRequest = {
        ...
        riId:this.riId,
        contactNo:+this.riName,
        AVDName:this.ilrCode1,
      ...
      let damagedsampleCollection = this.AvdPtService.addRiPt(this.ilrptListRequest)
      .subscribe(response => {
    ...
    editSubmit(editScForm: NgForm){
      ...
      console.log(editScForm.value.commentsdata.length==0)

      if(editScForm.value.commentsdata===undefined){
        this.commentsdata =  ' ';
      ...
      let damagedsampleCollection = this.AvdPtService.editAvd(this.ilrptListRequest)
      .subscribe(response => {
        this.addilrptResponse = response;
        if(this.addilrptResponse !== null){
          this.showResponseMessage(this.addilrptResponse.message, 's')
