...
@Component({
  selector: 'app-post-counselling-decision-no',
  templateUrl: './post-counselling-decision-no.component.html',
  styleUrls: ['./post-counselling-decision-no.component.css']
})
export class PostCounsellingDecisionNoComponent implements AfterViewInit, OnDestroy, OnInit {
    ...
    postpndtcounselledNoErrorMessage: string;
    ...
    ngOnInit() {
      ...
      let counselledyesdata = this.counsellednopostpndtService.getcounselledNoLists(this.counsellednopostpndtRequest)
        .subscribe(response => {
          ...
          if (this.counsellednopostpndtResponse !== null && this.counsellednopostpndtResponse.status === "true") {
            if (this.counsellednopostpndtResponse.data.length <= 0) {
              ...
              this.counsellednolists = this.counsellednopostpndtResponse.data;
              this.rerender();
            }
          }
          else {
    ...
    retrivecounsellednolists(){
      ...
      this.postpndtcounselledNoErrorMessage='';
      this.counsellednopostpndtRequest = {
    ...
    openpndtdetail(counselledyesdata: PostCounselledList ){
      ...
      this.router.navigateByUrl(`/app/update-post-pndtc-no?q=${this.anwSubjectId}`);
