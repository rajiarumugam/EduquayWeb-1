...
@Component({
  selector: 'app-mol-blood-sample-report-list',
  templateUrl: './molecule-blood-sample-recp.html',
  styleUrls: ['./molecule-blood-sample-recp.css']
})
export class MolecularBloodSampleReciptComponent implements AfterViewInit, OnDestroy, OnInit {
   ...
   if (subtab == 1) {
    ...
    let subProfile = this.SubjectProfileService.getNHMReportList(_obj)
      .subscribe(response => {
        ...
        {
          if (subtab == 1) {
            this.anmSubjectBadgeProfileListCount(1,1,18);

           }
           else if (subtab == 2) {
            this.anmSubjectBadgeProfileListCount(1,1,19);
           }
  ...
  anmSubjectBadgeProfileListCount(id,maintab,subtab) {
    ...
    let subProfile = this.SubjectProfileService.getNHMReportList(_obj)
      .subscribe(response => {
       ...
       if(maintab ===1)
       {
         console.log(this.tempsubtab,subtab,this.chcsamplingstatusCount,'TestCount',this.tempsubtab===18,this.tempsubtab===19,this.tempsubtab==18,this.tempsubtab==19);
            if( this.tempsubtab === 18)
                this.chcsamplingstatusCount = response['data'].length;

            if (this.tempsubtab === 19)
