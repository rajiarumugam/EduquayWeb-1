...
@Component({
  selector: 'app-chc-sample-pickpack-maldi1',
  templateUrl: './chc-sample-pickpack-maldi1.component.html',
  styleUrls: ['./chc-sample-pickpack-maldi1.component.css']
})
export class ChcSamplePickpackMaldi1Component implements AfterViewInit, OnDestroy, OnInit  {
  ...
  showResponseMessageMaldi(shipmentId: string, type: string) {
    var messageType = '';
    var title = `Shipment Id is ${shipmentId} and  ${shipmentId}-Maldi`;
    if (type === 'e') {
      Swal.fire({ allowOutsideClick: false, icon: 'error', title: shipmentId, confirmButtonText: 'Close' })
    } 
    else {
