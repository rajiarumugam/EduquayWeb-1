...
export class SSTUpdateNegativeComponent implements OnInit {
  ...
  searchBarCodetype()
  {
    ...
    if(_index >= 0)
    {
      ...
      this.showUploadResult = true;
      this.DataService.sendData(JSON.stringify({'screen':'SST','page':"received","positivecount":this.positiveList.length,"negativecount":this.negativeList.length,"receivedcount":this.tempCHCData.length, "module": "CHC- SAMPLE REC & PROCESS", "submodule": "Update SST Results", "pagealter": "Update Negative"}));
      this.rerender();
    } 
  }
  clicksearchBarcode()
      ...
      .subscribe(response => {
        ...
        if (this.chcUploadResponse !== null && this.chcUploadResponse.status === "true") {
            Swal.fire({ allowOutsideClick: false,
              ...
              var _tempNegativeLength = this.negativeList.length;
              this.negativeList = [];
              this.DataService.sendData(JSON.stringify({'screen':'SST','page':"received","positivecount":this.positiveList.length,"negativecount":this.negativeList.length,"receivedcount":this.chcReceiptsData.length-_tempNegativeLength-this.positiveList.length}));
