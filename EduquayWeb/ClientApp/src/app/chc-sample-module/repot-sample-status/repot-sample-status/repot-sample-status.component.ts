...
export class CHCreportSampleStatusComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  constructor(private PNDTCmasterService: PNDTCmasterService,private tokenService: TokenService,private route: ActivatedRoute,private PNDCService:PNDCService
    ,private dataservice: DataService,private router: Router,private _formBuilder: FormBuilder, private chcsampleService:chcsampleService,private loaderService: LoaderService
  ) { }

  ngOnInit() {

    this.dataservice.sendData(JSON.stringify({"module": "CHC- SAMPLE REC & PROCESS", "page": "Report - Sample Status"}));
  ...
  refreshData()
  {
    ...
    this.chcsampleService.getCHCSampleReport(_subjectObj) .subscribe(response => {
      console.log(response);
  ...
  exportexcel(): void 
    {
      this.fileName = "Report-samples.xlsx"
       ...
       let element = document.getElementById('report-samples'); 
