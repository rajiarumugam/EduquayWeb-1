...
@Component({
  selector: 'app-central-pick-pack-main',
  templateUrl: './central-pick-pack-main.component.html',
  styleUrls: ['./central-pick-pack-main.component.css']
})
export class CentralPickAndPackComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.subscription = this.DataService.receiveData().subscribe(x => { 
      if(JSON.parse(x).screen === "centralpickpack")
      {
