...
@Component({
  selector: 'app-patho-report-list',
  templateUrl: './patho-report-list.component.html',
  styleUrls: ['./patho-report-list.component.css']
})
export class PathoreportListComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  ngOnInit() {
    ...
    let subProfile = this.SubjectProfileService.getPathoReportList(_obj)
      .subscribe(response => {
  ...
  openpopup(index, subjectinfo){
    ...
    let subProfile = this.SubjectProfileService.getparticularanmSCHC(_obj)
      .subscribe(response => {
        if(response.subjectsDetail.length === 0)
        {
          this.loaderService.display(false);
            return;
        }
