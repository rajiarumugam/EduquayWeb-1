...
import { editBloodSamples, HplcPosBloodsamplesEditResponse, HplcPosBloodsamplesResponse } from 'src/app/shared/molecularlab-results/hplc-pos-bloodsamples/hplc-pos-bloodsamples-response';
...
export class EditResultsComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  retrieveBloodEditSamplesResponse: HplcPosBloodsamplesEditResponse;
  testedEditSamples: editBloodSamples[];

  constructor(
    private editSamplesServiceService: HplcPosBloodsamplesService,
  ...
  editResultSamples(molecularLabId) {
    ...
    this.editSamplesServiceService.geteditbloodSampleList(molecularLabId)
      .subscribe(response => {
        this.retrieveBloodEditSamplesResponse = response;
        this.loaderService.display(false);
        if (this.retrieveBloodEditSamplesResponse !== null && this.retrieveBloodEditSamplesResponse.status === "true") {
          if (this.retrieveBloodEditSamplesResponse.subjects.length <= 0) {
            this.editSamplesErrorMessage = response.message;  
          }
          else {
            this.testedEditSamples = this.retrieveBloodEditSamplesResponse.subjects;
