...
@Component({
  selector: 'app-block-aw-registration',
  templateUrl: './block-aw-registration.component.html',
  styleUrls: ['./block-aw-registration.component.css']
})

export class BlockAwRegistrationComponent implements OnInit {
  ...
  ngOnInit() {   
    ...
    this.dataservice.sendData(JSON.stringify({"module": "Block - REG & SAMPLING", "submodule": "Subject Registration", "page": "Antenatal Woman Registration"}));
    this.user = JSON.parse(this.tokenService.getUser('lu'));
    this.firstFormGroup = this._formBuilder.group({
      dor: ['', Validators.required],
      district: ['', Validators.required],
      anm: ['', Validators.required],
      ...
      rchid: ['', [Validators.required,Validators.pattern("^[0-9]*$"),
  ...
  getBlockData(){
    this.masterService.getuserBasedBlock(this.selectedDistrict)
    .subscribe(response => {
  ...
  getANMData(){
    ...
    this.anmList = response['data'];
      //this.selectedANM = this.user.blockId;
  ...
  getSCData(){
    this.masterService.getSCbyANM(this.selectedANM)
    .subscribe(response => {
  ...
  getRIData(){
    this.masterService.getRIbyANM(this.selectedANM)
    .subscribe(response => {
  ...
  getSC(){
    this.masterService.RetrieveSCByPHC(this.selectedphc)
    .subscribe(response => {
    ...
    dataBindinginServce()
    {
      ...
      var _obj = {
        "subjectPrimaryRequest": {
          ...
          "spouseGovIdTypeId": 0,
          "spouseGovIdDetail": "",
          "assignANMId": Number(this.selectedANM),
