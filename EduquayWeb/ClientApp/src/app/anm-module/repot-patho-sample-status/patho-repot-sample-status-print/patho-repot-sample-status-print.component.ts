...
export class PathoreportSampleStatusPrintComponentANM implements AfterViewInit, OnDestroy, OnInit {
  ...
  getCHCData(){
    if(this.selectedBlock != null)
    {
        ...
        .subscribe(response => {
          this.CHCdata1 = response['data'];
          this.CHCdata1.forEach(function(val,i){
          
            if(val.id ===this.user.chcId)
            {
              
              this.CHCdata.push(val);
            }
        },this);
  ...
  getANMData(){
    if(this.selectedchc != null)
    {
        ...
        .subscribe(response => {
          this.ANMdata1 = response['data'];
          this.loaderService.display(false);
          this.ANMdata1.forEach(function(val,i){
           console.log(val.id,this.user.id,val.id ===this.user.Id,typeof(this.user.id),typeof(val.id))
            if(val.id===this.user.id)
            {
              
              this.ANMdata.push(val);
            }

        },this);
        this.selectedAnm=this.user.id
