...
@Component({
  selector: 'app-hem-report-list',
  templateUrl: './state-report-list.component.html',
  styleUrls: ['./state-report-list.component.css']
})
export class HEMreportListComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  getBlockData(){
    ...
    this.PNDTCmasterService.getBlockByDistrict(this.selectedDistrict!=null?this.selectedDistrict:0)
    .subscribe(response => {
      console.log(response);
  ...
  anmSubjectProfileList(id,maintab,subtab) {
    console.log(id,maintab,subtab,'Before 2nd');
    if(maintab==1){
    subtab=4
    }
   else{
    ...
    console.log(subtab,"Value of subtab");
    ...
    let subProfile = this.SubjectProfileService.getNHMReportList(_obj)
      .subscribe(response => {
        ...
        if(maintab === 1)
        {
          console.log(JSON.stringify(this.subjectprofileLists),"Test")
  ...
  custumTabClick(i,j)
  {
      ...
      console.log("Before",this.maintabSelected,this.mainsubtabSelected);
